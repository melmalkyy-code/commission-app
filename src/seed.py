...
def _seed_tiers():
    ...
    for tier_name, targets in tier_targets.items():
        ...
        for cat_name, amount in targets.items():
            ...
            if cat:
                execute(
                    "INSERT INTO tier_category_targets (tier_id, category_id, target_amount) VALUES (%s, %s, %s) ON CONFLICT (tier_id, category_id) DO NOTHING",
...
def _seed_kpi():
    ...
    for name, weight, max_score, sort_order in items:
        ...
        if not existing:
            execute("INSERT INTO kpi_items (name, weight, max_score, sort_order) VALUES (%s, %s, %s, %s)", (name, weight, max_score, sort_order))
    ...
    if count and count['c'] == 0:
        for frm, to, mult, sort in rules:
            ...
            execute("INSERT INTO kpi_multiplier_rules (score_from, score_to, multiplier, is_unlimited, sort_order) VALUES (%s, %s, %s, %s, %s)", (frm, to, mult, unlimited, sort))
...
def _seed_brackets():
    ...
    for cat_name, brackets in bracket_data.items():
        ...
        for i, (frm, to, rate) in enumerate(brackets):
            execute(
                "INSERT INTO commission_brackets (category_id, from_amount, to_amount, commission_rate, is_unlimited, sort_order) VALUES (%s,%s,%s,%s,%s,%s)",
...
def _seed_salespersons():
    ...
    for name, br_name, tier_name in salespeople:
        ...
        tier = fetchone("SELECT id FROM target_tiers WHERE name=%s", (tier_name,))
        if br and tier:
            execute("INSERT INTO salespersons (name, branch_id, tier_id) VALUES (%s,%s,%s) ON CONFLICT (name) DO NOTHING", (name, br['id'], tier['id']))


def _seed_period():
    execute("INSERT INTO periods (year, quarter, is_current) VALUES (%s,%s,1) ON CONFLICT (year, quarter) DO NOTHING", (2026, 2))
...
def _seed_sample_data():
    ...
    for sp_name, cat_sales in sample_sales.items():
        ...
        for cat_name, amount in cat_sales.items():
            ...
            if cat:
                execute(
                    "INSERT INTO sales_records (period_id, salesperson_id, category_id, actual_sales) VALUES (%s,%s,%s,%s) ON CONFLICT (period_id, salesperson_id, category_id) DO NOTHING",
